...
function fnDisplayTransactionList(dataList) {
    ...
    if (dataList != undefined && ((dataList.length && dataList[0].fault) || dataList.fault)) {
        ...
        let strTransaction = "<br><h3>&nbsp;&nbsp;&#x25A0 &nbsp;Transaction List</h3>";
        strTransaction += " <table><tr><th>Sel<th>Transaction Id<th>User Id<th>Book Id<th>Book Title";
        strTransaction += "<th>Issue Date<th>Returned</th></tr>";
        //console.log("dataList.length = " + dataList.length);

        if (dataList.length == undefined) { // when a record exists by ID
            strTransaction += setRow(dataList, 0, 0); // dataItem, dataMode [0,1,2], counter/index)
        } else if (dataList.length == 1) { // when a record exists by Name
            strTransaction += setRow(dataList[0], 1, 0);
            ...
            dataList.forEach(data => {
                ...
                strTransaction += setRow(data, 2, counter);
                counter++;
            });
        }
        resultDivList.innerHTML += strTransaction + "</table>";
    }
...
function setRow(dataItem, dataMode, counter) {
    ...
    strRow += "<td>" + dataItem.user.userId + "<td>" + dataItem.book.bookId + "<td>" + dataItem.book.bookInfo.title;
