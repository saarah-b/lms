...
function fnDisplayDeleteResponse(data) {
    ...
    if (data.slice(0, 5).toLowerCase() === "error") {
        ...
        strFault += "<tr><th>Path</td><td style='color:red'>" + apiContext + document.getElementById('tdUserId').innerText + "</td></tr></table>";
        ...
        fnResetSrch();
        document.getElementById("btnDeleteUser").className = "dbtn";
...
function fnDisplayUserList(dataList) {
    ...
    if (dataList.fault) {
        ...
        if (dataList.length == undefined) { // when a record exists by ID
            ...
            strDtl += "<br><button id='btnDeleteUser' onclick='fnDeleteUser();'>Delete User</button> &nbsp; <button onclick='fnReset();'>Reset</button>";
            strDtl += "&nbsp; <button id='btnDeleteAnotherUser' onclick='fnDeleteAnotherUser();' class='dbtn'>Delete Another User</button> ";
        }
...
function fnDeleteAnotherUser() {
    document.getElementById("btnDeleteUser").className = "";
