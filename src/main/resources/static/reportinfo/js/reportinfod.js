...
function fnDisplayDeleteResponse(data) {
    ...
    if (data.slice(0, 5).toLowerCase() === "error") {
        ...
        strFault += "<tr><th>Path</td><td style='color:red'>" + apiContext + document.getElementById('tdReportInfoId').innerText + "</td></tr></table>";
        ...
        fnResetSrch();
        document.getElementById("btnDeleteReportInfo").className = "dbtn";
...
function fnDisplayReportInfoList(dataList) {
    ...
    if (dataList.fault) {
        ...
        if (dataList.length == undefined) { // when a record exists by ID
            ...
            strDtl += "<br><button id='btnDeleteReportInfo' onclick='fnDeleteReportInfo();'>Delete ReportInfo</button> &nbsp; <button onclick='fnReset();'>Reset</button>";
            strDtl += "&nbsp; <button id='btnDeleteAnotherReportInfo' onclick='fnDeleteAnotherReportInfo();' class='dbtn'>Delete Another ReportInfo</button> ";
        }
...
function fnDeleteAnotherReportInfo() {
    document.getElementById("btnDeleteReportInfo").className = "";
