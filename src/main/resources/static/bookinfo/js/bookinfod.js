...
/*
This function gets the Book Info ID on the page and submits the information to the server as an API request.
...
function fnDisplayDeleteResponse(data) {
    ...
    if (data.slice(0, 5).toLowerCase() === "error") {
        ...
        strFault += "<tr><th>Path</td><td style='color:red'>" + apiContext + document.getElementById('tdBookInfoId').innerText + "</td></tr></table>";
        ...
        fnResetSrch();
        document.getElementById("btnDeleteBookInfo").className = "dbtn";
...
function fnDisplayBookInfoList(dataList) {
    ...
    if (dataList.fault) {
        ...
        if (dataList.length == undefined) { // when a record exists by ID
            ...
            strDtl += "<br><button id='btnDeleteBookInfo' onclick='fnDeleteBookInfo();'>Delete BookInfo</button> &nbsp; <button onclick='fnReset();'>Reset</button>";
            strDtl += "&nbsp; <button id='btnDeleteAnotherBookInfo' onclick='fnDeleteAnotherBookInfo();' class='dbtn'>Delete Another BookInfo</button> ";
        }
...
function fnDeleteAnotherBookInfo() {
    document.getElementById("btnDeleteBookInfo").className = "";
