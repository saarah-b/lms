...
function fnDisplayDeleteResponse(data) {
    ...
    if (data.slice(0, 5).toLowerCase() === "error") {
        ...
        strFault += "<tr><th>Path</td><td style='color:red'>" + apiContext + document.getElementById('tdBookId').innerText + "</td></tr></table>";
        ...
        fnResetSrch();
        document.getElementById("btnDeleteBook").className = "dbtn";
...
function fnDisplayBookList(dataList) {
    ...
    if (dataList.fault) {
        ...
        if (dataList.length == undefined) { // when a record exists by ID
            strDtl += "<table><tr><th width=30%>Book Id<td id='tdBookId'>" + dataList.bookId + "<tr><th>Shelf Reference <td>" + dataList.shelfReference;
            strDtl += "<tr><th>Location <td>" + dataList.location + "<tr><th>Edition <td>" + dataList.edition;
            strDtl += "<tr><th>Available <td>" + dataList.available + "</table>";
            strDtl += "<br><button id='btnDeleteBook' onclick='fnDeleteBook();'>Delete Book</button> &nbsp; <button onclick='fnReset();'>Reset</button>";
            strDtl += "&nbsp; <button id='btnDeleteAnotherBook' onclick='fnDeleteAnotherBook();' class='dbt'>Delete Another Book</button> ";
        }
...
function fnDeleteAnotherBook() {
        document.getElementById("btnDeleteBook").className = "";
